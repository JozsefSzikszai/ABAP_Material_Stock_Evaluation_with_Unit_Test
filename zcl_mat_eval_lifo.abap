CLASS zcl_mat_eval_lifo DEFINITION
  PUBLIC
...
CLASS zcl_mat_eval_lifo IMPLEMENTATION.


* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_MAT_EVAL_LIFO->ZIF_MAT_EVAL~EVALUATE
* +-------------------------------------------------------------------------------------------------+
* +--------------------------------------------------------------------------------------</SIGNATURE>
  METHOD zif_mat_eval~evaluate.
    ...
    LOOP AT mt_material_movements
      ...
      CASE <ls_material_movement>-shkzg.
        ...
        WHEN 'H'.
          " if it is a negative stock movement, we subtract it from stock always starting at the last line
          ...
          WHILE lv_menge GT 0.
            ...
            IF <ls_result> IS ASSIGNED.
              IF <ls_result>-menge GT lv_menge.
                <ls_result>-menge -= lv_menge.
                lv_menge = 0.
              ELSE.
                lv_menge -= <ls_result>-menge.
                DELETE mt_result INDEX lines( mt_result ).
              ENDIF.
              UNASSIGN <ls_result>.
            ENDIF.
          ENDWHILE.
